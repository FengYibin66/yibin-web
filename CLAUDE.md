...
| 位置 | 数量 | 内容 |
|------|------|------|
| `apps/resume/__tests__/` | 1567（100 文件） | 组件与逻辑单测（vitest）：含走廊 domain 的纯函数（狗 reducer 2000 步随机不变量、气泡规则、时间线落点、世界时钟、路线计划）与 AST 门禁 |
| `apps/resume/e2e/` | 176（88 spec ×2 形态） | Playwright E2E（chromium + mobile-safari）：静态导出形态（含「产物里每个 woff2 以 `wOF2` 魔数开头」的字体门禁，ADR 20260909163155）+ Lab 的行为（进房 / 退房 / 传送 / ESC / 面板 / 教程 / 语言切换 / 走廊世界状态 / 活物 / 招聘官路线）+ Classic 滚动显形的全部进入路径。Lab 那批的坑见 `apps/resume/AGENTS.md` 与 `apps/resume/scripts/qa/AGENTS.md` |
