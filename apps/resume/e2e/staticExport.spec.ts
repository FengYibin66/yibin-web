...
test.describe('Classic 顶栏导航（ADR 20260909182319 第三批）', () => {
  /*
    守的是「功能在某一档消失」。**jsdom 不算 CSS**——`hidden md:flex` 与
    `md:hidden` 在单测里都无效、两组元素都在 DOM 里，所以「哪一档真的看得见、
    点得到」只能在这里量渲染结果。结构与行为归 `__tests__/navbarMenu.test.tsx`。
  */
  ...
  test('顶栏所有可点元素的触摸目标不小于 44 高', async ({ page }) => {
    ...
    /*
      量渲染结果而不是源码里的数字：小于 44 的形态有三种（内联方形声明、
      padding 撑出来的文字链接、只有 padding 的关闭按钮），AST 只认第一种。
    */
    /*
      判据范围：**控件**（按钮）两档都要 ≥44，**窄屏菜单里的行**要 ≥44，
      **宽屏平铺的文字链接豁免**（实测高 20px，但在 hover 指针下——WCAG 2.5.8
      对行内文字链接明确豁免，44 是 2.5.5 针对触摸目标的要求）。

      套到桌面 hover 链接上等于为一个不存在的问题重做桌面顶栏，
      而那种误报会训练人给判据加豁免。
    */
