...
| [`20260822120808`](./20260822120808-portal-types-derived-from-schema.md) | Portal 接口类型从 Drizzle schema 派生，不手写 | 已接受 | portal 的 `Profile` / `Project` 类型由 `schema.ts` 经 Drizzle `$inferSelect` 派生并置于共享位置，前后端同源；不引入 OpenAPI codegen（规模不匹配） |
