...
| # | 选择 | 备选 | 倾向 |
|---|------|------|------|
| A | **走廊世界状态的持有形态** | zustand store（selector + transient）/ 扩 `SceneContext` / 模块级可变对象 + 事件 | zustand；理由 §2.3 |
| B | **地标统一表** | 一张判别联合表 / 保持多张常量表 + 一个聚合视图 / 每种地标一个模块各自导出 | 一张表；理由 §3 |
| C | **显形是策略还是特例** | `inkLevel` 统一策略 / 三处各写 | 统一策略；理由 §4 |
| D | **加载期让走廊可见** | 纸半透明 / 提前撕开 / 保持全遮（放弃"画出走廊"） | 提前撕开到 `progress > 0.3`，之后纸外沿继续显示进度 |
| E | **模式互斥用状态机还是布尔** | 接线 `corridor.machine` 加 `touring` / 再加一个布尔 | 状态机；20260903211338 已决定"接线不删除"，这里只是加一个状态 |
| F | **时间线映射** | 第 0 段 2014–2026 线性 / 每段一个时代 / 不做刻度只做涂鸦 | 第 0 段线性 |
| G | **窗外有颜色** | 允许（窗外是"外面"）/ 全部米色系 | 允许，但饱和度压到与门贴纸同级；截图定 |

活物（`20260908160918`）已单独成 ADR。A + B + C 可以合成一份「走廊世界状态、地标表与显形策略」，因为三者互相依赖（显形读世界状态、按地标 id 记忆）；D/G 是产品决定写进规格；E/F 各一份小 ADR。
...
| 期 | 内容 | 依赖 | 用户可见 |
|----|------|------|----------|
| 0 基础 | `corridorStore` + 导轨发布 z/v；`motion.ts` 接入 Doodles / BugEaster；地标表重构（视觉零变化）；`lab-walkthrough.mjs` | ADR A+B+C | 无（reduced 用户除外） |
| 1 显形与记忆 | `ink.ts`；加载期画出走廊；看过永久上色（`memory.ts`）；地图变手绘平面图 + 未访问问号 | 0 | 加载体验、回访差异、地图 |
| 2 活物与说话 | 狗、猫、`speech.ts`、头像台词 | 0；活物 ADR | 走廊有人陪 |
| 3 时间线与窗 | `years` 字段 + 门禁；年份刻度 + 墙面涂鸦；三扇窗 + 世界时钟；第二圈变体 | 1 | 走廊会讲履历 |
| 4 路线 | 接线 `corridor.machine` + `touring`；导轨 `scrollTo/hold`；"带我走一遍" | 0、1 | 60 秒看完 |
| 5 声 | 脚步、叫声、纸声 | 2 | 听得见 |
