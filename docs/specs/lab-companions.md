...
实现：`Text`（troika）+ `fontForText()`，文案进 `labUi.companions`，中英键一致；reduced 下照常显示（文字不是运动）。

## 3. 猫：行为

- **位置**：第 0 段柜子（`CORRIDOR_FURNITURE` 中 `cabinet`，relativeZ −49、右墙）顶面，靠相框内侧。**只在第 0 段**（无限走廊的后续段不再出现——猫是一个具体的存在，不是壁纸）。
- **叙事**：相框里是 `beloved.jpg`，猫守着它。教程气泡文案："猫在守着一张照片"（en："The cat is guarding a photo"）。
- **状态**：
