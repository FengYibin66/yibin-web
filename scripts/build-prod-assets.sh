...
echo ""
echo "⚠️  If nginx is already running, remount volumes or it will keep serving"
echo "   the old (empty) directories → 403/404:"
echo "   docker compose --env-file .env.production -f docker-compose.prod.yml up -d --force-recreate nginx"
echo "   (./scripts/deploy-prod.sh does this automatically)"
